...
class FaceIdentifier(Module):
    ...
    def __init__(
        ...
        if len(output_shape) not in (2, 4):
            raise RuntimeError(
                "The model expects output shape [1, n, 1, 1] or [1, n], got {}".format(  # noqa: E501
                    output_shape,
                ),
            )
    ...
    def preprocess(self, frame, rois, landmarks):
        ...
        checkpoint = self._align_rois(inputs, landmarks)
    ...
    def postprocess(self):
        ...
        if len(descriptors) != 0:
            matches = self.faces_database.match_faces(
                descriptors,
                self.match_algo,
            )
    ...
    def get_transform(src, dst):
        ...
        ), f"2d input arrays are expected, got {src.shape}"
        ...
        transform[:, 2] = dst_col_mean.T - np.matmul(
            transform[:, 0:2],
            src_col_mean.T,
        )
        return transform
    ...
    def _align_rois(self, face_images, face_landmarks):
        ...
        assert len(face_images) == len(
            face_landmarks,
        ), "Input lengths differ, got {} and {}".format(
            len(face_images),
            len(face_landmarks),
        )
        ...
        for image, image_landmarks in zip(face_images, face_landmarks):
            ...
            transform = FaceIdentifier.get_transform(
                desired_landmarks,
                landmarks,
            )
            cv2.warpAffine(
